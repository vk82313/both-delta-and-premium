...
# Global thresholds for arbitrage system (modified defaults)
...
class PremiumMatchConfig:
    ...
    btc_min_premium: float = 2.0      # new default
    eth_min_premium: float = 0.05     # new default
...
class PremiumTrackerConfig:
    ...
    last_alert_time: float = 0.0   # not used anymore
...
# -------------------------------
# System 5: Premium Tracker Functions (NO COOLDOWN)
# -------------------------------
        ...
        try:
            ...
            if symbol and best_bid is not None and best_ask is not None:
                ...
                if current_time - self.last_arbitrage_check >= PROCESS_INTERVAL:
                    ...
                    check_system5_eth(self)  # no cooldown
        ...
        while self.running:
            try:
                ...
                if current_time - self.last_arbitrage_check >= PROCESS_INTERVAL:
                    ...
                    check_system5_btc(self)  # no cooldown
                    self.last_arbitrage_check = current_time
                    global last_check_time
                    last_check_time = datetime.now()
...
# -------------------------------
# Dark mode HTML Template (updated defaults displayed)
# -------------------------------
HTML_TEMPLATE = '''
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .container {
            max-width: 1200px; margin: 0 auto; background: #1e1e2f;
            border-radius: 20px; box-shadow: 0 20px 60px rgba(0,0,0,0.5); overflow: hidden;
        }
        .header {
        ...
        .tabs {
            display: flex; background: #2a2a3f; border-bottom: 2px solid #3a3a5a; flex-wrap: wrap;
        }
        .tab-btn {
            flex: 1; padding: 20px; border: none; background: none;
            font-size: 1.1rem; font-weight: 600; cursor: pointer; transition: all 0.3s ease; color: #999;
        }
        .tab-btn:hover { background: #3a3a5a; color: #ccc; }
        .tab-btn.active { background: #1e1e2f; color: #4a6ee0; border-bottom: 3px solid #4a6ee0; }
        .tab-content { display: none; padding: 30px; }
        .tab-content.active { display: block; }
        .alert-success {
            background: #2e4a2e; color: #81c784; padding: 15px; border-radius: 10px;
            margin-bottom: 20px; border: 1px solid #4caf50;
        }
        .system-section { margin-bottom: 40px; }
        .section-title {
            font-size: 1.5rem; margin-bottom: 20px; color: #5c6bc0;
        ...
        .stat-card {
            background: #2a2a3f; padding: 25px; border-radius: 15px; border-left: 5px solid #5c6bc0;
        }
        .stat-card h3 { color: #e0e0e0; margin-bottom: 15px; font-size: 1.3rem; }
        .stat-item {
            margin-bottom: 10px; font-size: 1.1rem; display: flex; justify-content: space-between;
        }
        .stat-label { color: #999; }
        .stat-value { font-weight: 600; color: #e0e0e0; }
        .threshold-card {
            background: #2a2a3f; padding: 25px; border-radius: 15px;
            box-shadow: 0 5px 20px rgba(0,0,0,0.3); margin-bottom: 20px;
        }
        .threshold-card h3 { color: #e0e0e0; margin-bottom: 20px; font-size: 1.3rem; }
        .threshold-input {
            width: 100%; padding: 12px; font-size: 1.1rem; border: 2px solid #3a3a5a;
            border-radius: 10px; margin-bottom: 15px; background: #1e1e2f; color: #e0e0e0;
            transition: all 0.3s ease;
        }
        .threshold-input:focus {
            outline: none; border-color: #4a6ee0; box-shadow: 0 0 0 3px rgba(74, 110, 224, 0.2);
        }
        .update-btn {
            padding: 15px 30px; font-size: 1.1rem; font-weight: 600; border: none;
            border-radius: 10px; cursor: pointer; transition: all 0.3s ease;
            background: linear-gradient(135deg, #4a6ee0, #6a11cb); color: white; width: 100%;
        }
        .update-btn:hover { transform: translateY(-2px); box-shadow: 0 8px 25px rgba(74, 110, 224, 0.4); }
        .option-section {
            display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr));
            gap: 20px; margin-bottom: 30px;
        }
        .option-card {
            background: #2a2a3f; padding: 25px; border-radius: 15px; border-top: 5px solid;
        }
        .btc-call { border-color: #3498db; } .btc-put { border-color: #e74c3c; }
        .eth-call { border-color: #2ecc71; } .eth-put { border-color: #9b59b6; }
        .option-card h4 { font-size: 1.2rem; margin-bottom: 15px; color: #e0e0e0; }
        .select-input {
            width: 100%; padding: 12px; font-size: 1.1rem; border: 2px solid #3a3a5a;
            border-radius: 10px; margin-bottom: 15px; background: #1e1e2f; color: #e0e0e0;
        }
        .checkbox-group { display: flex; align-items: center; gap: 10px; margin-top: 15px; color: #ccc; }
        .checkbox-group input[type="checkbox"] { width: 20px; height: 20px; accent-color: #4a6ee0; }
        .activate-btn {
            padding: 20px; font-size: 1.3rem; font-weight: 700; border: none; border-radius: 15px;
            cursor: pointer; transition: all 0.3s ease; background: linear-gradient(135deg, #2ecc71, #27ae60);
            color: white; width: 100%; margin-top: 20px;
        }
        .activate-btn:hover { transform: translateY(-3px); box-shadow: 0 12px 30px rgba(46, 204, 113, 0.4); }
        .status-panel { background: #2a2a3f; padding: 25px; border-radius: 15px; margin-top: 30px; }
        .status-panel h3 { color: #e0e0e0; margin-bottom: 20px; font-size: 1.3rem; }
        .status-item {
            display: flex; justify-content: space-between; align-items: center;
            padding: 12px 0; border-bottom: 1px solid #3a3a5a;
        }
        .status-item:last-child { border-bottom: none; }
        .status-label { font-size: 1.1rem; color: #999; }
        .status-value { font-weight: 600; font-size: 1.1rem; }
        .status-active { color: #2ecc71; } .status-inactive { color: #e74c3c; }
        ...
        .condition-1 { background: linear-gradient(135deg, #3498db, #2980b9); color: white; }
        .condition-2 { background: linear-gradient(135deg, #9b59b6, #8e44ad); color: white; }
        .condition-panel h3 { color: white; margin-bottom: 20px; font-size: 1.5rem; display: flex; align-items: center; gap: 10px; }
        .condition-status {
            display: flex; justify-content: space-between; align-items: center;
            margin-bottom: 20px; padding: 15px; background: rgba(255, 255, 255, 0.1); border-radius: 10px;
        }
        .condition-controls { margin-top: 20px; }
        .control-buttons { display: flex; gap: 10px; margin-top: 15px; }
        .start-btn {
            padding: 15px; font-size: 1.1rem; font-weight: 600; border: none; border-radius: 10px;
            cursor: pointer; background: #2ecc71; color: white; flex: 1; transition: all 0.3s ease;
        }
        .stop-btn {
            padding: 15px; font-size: 1.1rem; font-weight: 600; border: none; border-radius: 10px;
            cursor: pointer; background: #e74c3c; color: white; flex: 1; transition: all 0.3s ease;
        }
        .start-btn:hover { background: #27ae60; transform: translateY(-2px); }
        .stop-btn:hover { background: #c0392b; transform: translateY(-2px); }
        .config-section { background: #2a2a3f; padding: 25px; border-radius: 15px; margin-bottom: 20px; }
        .config-section h4 { color: #e0e0e0; margin-bottom: 20px; font-size: 1.3rem; }
        .condition-section {
            background: #1e1e2f; padding: 20px; border-radius: 10px; margin-bottom: 20px; border-left: 4px solid;
        }
        .condition-1-section { border-left-color: #3498db; }
        .condition-2-section { border-left-color: #9b59b6; }
        .condition-section h5 { font-size: 1.1rem; margin-bottom: 15px; color: #e0e0e0; }
        .condition-section small { color: #999; display: block; margin-top: 5px; font-size: 0.9rem; }
        .config-row { margin-bottom: 20px; }
        .config-row label { display: block; margin-bottom: 8px; font-weight: 600; color: #ccc; }
        .checkbox-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 15px; margin-top: 20px; }
        .save-btn {
            padding: 15px 30px; font-size: 1.1rem; font-weight: 600; border: none; border-radius: 10px;
            cursor: pointer; background: #3498db; color: white; width: 100%; margin-top: 20px;
        }
        .save-btn:hover { background: #2980b9; transform: translateY(-2px); }
        .cooldown-note {
            background: rgba(255, 255, 255, 0.2); color: white; padding: 10px; border-radius: 5px;
            margin-top: 10px; font-size: 0.9rem; text-align: center;
        }
        .system4-panel { background: linear-gradient(135deg, #f39c12, #e67e22); color: white; }
        .cooldown-control { display: flex; gap: 10px; align-items: center; justify-content: center; margin-top: 15px; }
        .cooldown-btn {
            padding: 10px 20px; font-size: 1.2rem; font-weight: bold; border: none; border-radius: 8px;
            cursor: pointer; background: rgba(255, 255, 255, 0.2); color: white; transition: all 0.3s ease;
        }
        .cooldown-btn:hover { background: rgba(255, 255, 255, 0.3); transform: scale(1.05); }
        .cooldown-input {
            width: 100px; text-align: center; padding: 10px; font-size: 1.1rem; border: 2px solid white;
            border-radius: 8px; background: rgba(255, 255, 255, 0.9); color: #333; font-weight: bold;
        }
        .filter-input {
            width: 150px; text-align: center; padding: 10px; font-size: 1.1rem; border: 2px solid #f39c12;
            border-radius: 8px; background: white; color: #333; font-weight: bold;
        }
        .stat-highlight { font-size: 1.5rem; font-weight: bold; color: #f39c12; }
        .tracker-grid {
            display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px; margin-bottom: 30px;
        }
        .tracker-card {
            background: #2a2a3f; padding: 25px; border-radius: 15px; border-top: 5px solid; text-align: center;
        }
        .tracker-card.monitoring { background: #2e4a2e; box-shadow: 0 0 20px rgba(46, 204, 113, 0.2); }
        .tracker-card h4 { font-size: 1.3rem; margin-bottom: 20px; color: #e0e0e0; }
        .tracker-status { font-size: 1.2rem; font-weight: 700; margin: 15px 0; }
        .footer {
            text-align: center; padding: 20px; color: #999; border-top: 1px solid #3a3a5a; margin-top: 30px;
        }
        @media (max-width: 768px) {
            .header h1 { font-size: 2rem; } .tab-btn { padding: 15px; font-size: 0.9rem; }
            .tab-content { padding: 20px; } .stats-grid { grid-template-columns: 1fr; }
            .option-section { grid-template-columns: 1fr; } .tracker-grid { grid-template-columns: 1fr; }
            .control-buttons { flex-direction: column; } .checkbox-grid { grid-template-columns: 1fr; }
            .cooldown-control { flex-wrap: wrap; }
        }
    </style>
</head>
<body>
    <div class="container">
            ...
            <div class="system-section">
                ...
                <div class="stats-grid">
                    <div class="stat-card">
                        ...
                        <div class="stat-item"><span class="stat-label">Active Expiry:</span><span class="stat-value">{{ eth_bot.active_expiry }}</span></div>
                    ...
                    <div class="stat-card">
                        ...
                        <div class="stat-item"><span class="stat-label">Active Expiry:</span><span class="stat-value">{{ btc_bot.active_expiry }}</span></div>
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px;">
                        <div>
                            ...
                            <form action="/update_eth_threshold" method="POST">
                                <input type="number" name="threshold" value="{{ "%.2f"|format(DELTA_THRESHOLD['ETH']) }}" step="0.01" min="0.01" max="10" class="threshold-input" required>
                        ...
                        <div>
                            ...
                            <form action="/update_btc_threshold" method="POST">
                                <input type="number" name="threshold" value="{{ "%.2f"|format(DELTA_THRESHOLD['BTC']) }}" step="0.01" min="0.01" max="50" class="threshold-input" required>
                    ...
                    <div class="option-section">
                        <div class="option-card btc-call">
                            ...
                            <select name="btc_call_strike" class="select-input">
                                ...
                                <option value="{{ strike }}" {% if alert_configs['btc_call'].strike == strike %}selected{% endif %}>{{ strike }}</option>
                                {% endfor %}
                            </select>
                            ...
                            <div class="checkbox-group">
                                <input type="checkbox" name="btc_call_monitor" id="btc_call_monitor" {% if alert_configs['btc_call'].is_monitoring %}checked{% endif %}>
                        ...
                        <div class="option-card btc-put">
                            ...
                            <select name="btc_put_strike" class="select-input">
                                ...
                                <option value="{{ strike }}" {% if alert_configs['btc_put'].strike == strike %}selected{% endif %}>{{ strike }}</option>
                                {% endfor %}
                            </select>
                            ...
                            <div class="checkbox-group">
                                <input type="checkbox" name="btc_put_monitor" id="btc_put_monitor" {% if alert_configs['btc_put'].is_monitoring %}checked{% endif %}>
                        ...
                        <div class="option-card eth-call">
                            ...
                            <select name="eth_call_strike" class="select-input">
                                ...
                                <option value="{{ strike }}" {% if alert_configs['eth_call'].strike == strike %}selected{% endif %}>{{ strike }}</option>
                                {% endfor %}
                            </select>
                            ...
                            <div class="checkbox-group">
                                <input type="checkbox" name="eth_call_monitor" id="eth_call_monitor" {% if alert_configs['eth_call'].is_monitoring %}checked{% endif %}>
                        ...
                        <div class="option-card eth-put">
                            ...
                            <select name="eth_put_strike" class="select-input">
                                ...
                                <option value="{{ strike }}" {% if alert_configs['eth_put'].strike == strike %}selected{% endif %}>{{ strike }}</option>
                                {% endfor %}
                            </select>
                            ...
                            <div class="checkbox-group">
                                <input type="checkbox" name="eth_put_monitor" id="eth_put_monitor" {% if alert_configs['eth_put'].is_monitoring %}checked{% endif %}>
                    ...
                    <form action="/update_spike_config" method="POST">
                        <div class="condition-section condition-1-section">
                            ...
                            <div class="config-row">
                                ...
                                <input type="number" id="min_spike_percent" name="min_spike_percent" value="{{ spike_config.min_spike_percent }}" step="0.1" class="threshold-input" required>
                            ...
                            <div class="config-row">
                                ...
                                <input type="number" id="spike_min_premium" name="spike_min_premium" value="{{ spike_config.spike_min_premium }}" step="0.01" min="0" class="threshold-input" required>
                        ...
                        <div class="condition-section condition-2-section">
                            ...
                            <div class="config-row">
                                ...
                                <input type="number" id="min_spread_percent" name="min_spread_percent" value="{{ spike_config.min_spread_percent }}" step="0.1" class="threshold-input" required>
                            ...
                            <div class="config-row">
                                ...
                                <input type="number" id="spread_min_premium" name="spread_min_premium" value="{{ spike_config.spread_min_premium }}" step="0.01" min="0" class="threshold-input" required>
                                <small>Only check spreads for options with bid price ≥ this amount ($)</small>
                            </div>
                        </div>
                        <div class="checkbox-grid">
                            <div class="checkbox-group">
                                <input type="checkbox" id="monitor_eth" name="monitor_eth" {% if spike_config.monitor_eth %}checked{% endif %}>
                                <label for="monitor_eth">Monitor ETH</label>
                            </div>
                            <div class="checkbox-group">
                                <input type="checkbox" id="monitor_btc" name="monitor_btc" {% if spike_config.monitor_btc %}checked{% endif %}>
                                <label for="monitor_btc">Monitor BTC</label>
                            </div>
                            <div class="checkbox-group">
                                <input type="checkbox" id="monitor_calls" name="monitor_calls" {% if spike_config.monitor_calls %}checked{% endif %}>
                                <label for="monitor_calls">Include Calls</label>
                            </div>
                            <div class="checkbox-group">
                                <input type="checkbox" id="monitor_puts" name="monitor_puts" {% if spike_config.monitor_puts %}checked{% endif %}>
                ...
                <div class="condition-panel system4-panel">
                    ...
                    <div class="condition-controls" style="display: flex; gap: 15px; margin-top: 20px;">
                        <form action="/start_system4" method="POST" style="flex: 1;">
                            <button type="submit" class="start-btn" style="background: #2ecc71; width: 100%;">▶️ START SYSTEM 4</button>
                        </form>
                        <form action="/stop_system4" method="POST" style="flex: 1;">
                            <button type="submit" class="stop-btn" style="background: #e74c3c; width: 100%;">⏸️ STOP SYSTEM 4</button>
                        </form>
                    </div>
                </div>
                    ...
                    <form action="/update_system4_cooldown" method="POST">
                        <div class="config-row">
                            ...
                            <div class="cooldown-control">
                                ...
                                <input type="number" id="cooldown_seconds" name="cooldown_seconds" value="{{ premium_match_config.cooldown_seconds }}" step="5" min="5" max="300" class="cooldown-input">
                                <button type="button" onclick="incrementCooldown()" class="cooldown-btn">+</button>
                                <span style="margin-left: 10px; color: white;">seconds</span>
                            </div>
                        </div>
                    ...
                    <form action="/update_system4_btc_filter" method="POST">
                        <div class="config-row">
                            ...
                            <div class="cooldown-control">
                                ...
                                <input type="number" id="btc_min_premium" name="btc_min_premium" value="{{ premium_match_config.btc_min_premium }}" step="0.5" min="0" max="1000" class="filter-input">
                    ...
                    <form action="/update_system4_eth_filter" method="POST">
                        <div class="config-row">
                            ...
                            <div class="cooldown-control">
                                ...
                                <input type="number" id="eth_min_premium" name="eth_min_premium" value="{{ premium_match_config.eth_min_premium }}" step="0.01" min="0.01" max="500" class="filter-input">
                    ...
                    <div class="tracker-card btc-call {% if premium_tracker_configs['btc_call'].active %}monitoring{% endif %}">
                        ...
                        <form action="/start_system5_btc_call" method="POST">
                            <select name="strike" class="select-input" {% if premium_tracker_configs['btc_call'].active %}disabled{% endif %}>
                                <option value="">Select Strike</option>
                                {% for strike in btc_bot.option_chain_data.calls.keys()|sort %}
                                <option value="{{ strike }}" {% if premium_tracker_configs['btc_call'].strike == strike %}selected{% endif %}>{{ strike }}</option>
                                {% endfor %}
                            </select>
                    ...
                    <div class="tracker-card btc-put {% if premium_tracker_configs['btc_put'].active %}monitoring{% endif %}">
                        ...
                        <form action="/start_system5_btc_put" method="POST">
                            <select name="strike" class="select-input" {% if premium_tracker_configs['btc_put'].active %}disabled{% endif %}>
                                <option value="">Select Strike</option>
                                {% for strike in btc_bot.option_chain_data.puts.keys()|sort %}
                                <option value="{{ strike }}" {% if premium_tracker_configs['btc_put'].strike == strike %}selected{% endif %}>{{ strike }}</option>
                                {% endfor %}
                            </select>
                    ...
                    <div class="tracker-card eth-call {% if premium_tracker_configs['eth_call'].active %}monitoring{% endif %}">
                        ...
                        <form action="/start_system5_eth_call" method="POST">
                            <select name="strike" class="select-input" {% if premium_tracker_configs['eth_call'].active %}disabled{% endif %}>
                                <option value="">Select Strike</option>
                                {% for strike in eth_bot.option_chain_data.calls.keys()|sort %}
                                <option value="{{ strike }}" {% if premium_tracker_configs['eth_call'].strike == strike %}selected{% endif %}>{{ strike }}</option>
                                {% endfor %}
                            </select>
                    ...
                    <div class="tracker-card eth-put {% if premium_tracker_configs['eth_put'].active %}monitoring{% endif %}">
                        ...
                        <form action="/start_system5_eth_put" method="POST">
                            <select name="strike" class="select-input" {% if premium_tracker_configs['eth_put'].active %}disabled{% endif %}>
                                <option value="">Select Strike</option>
                                {% for strike in eth_bot.option_chain_data.puts.keys()|sort %}
                                <option value="{{ strike }}" {% if premium_tracker_configs['eth_put'].strike == strike %}selected{% endif %}>{{ strike }}</option>
                                {% endfor %}
                            </select>
        ...
        <div class="footer">
            ...
            <p>Last Update: {{ get_ist_time() }} • <a href="/health" style="color: #5c6bc0;">Health Check</a></p>
        </div>
    </div>
    <script>
        ...
        function decrementCooldown() {
            ...
            if (currentValue > 5) input.value = currentValue - 5;
        }
        function incrementCooldown() {
            let input = document.getElementById('cooldown_seconds');
            let currentValue = parseInt(input.value);
            if (currentValue < 300) input.value = currentValue + 5;
        }
        function decrementBtcFilter() {
            let input = document.getElementById('btc_min_premium');
            let currentValue = parseFloat(input.value);
            if (currentValue >= 0.5) input.value = (currentValue - 0.5).toFixed(1);
            else if (currentValue > 0 && currentValue < 0.5) input.value = 0;
        }
        function incrementBtcFilter() {
            let input = document.getElementById('btc_min_premium');
            let currentValue = parseFloat(input.value);
            if (currentValue <= 999.5) input.value = (currentValue + 0.5).toFixed(1);
        }
        function decrementEthFilter() {
            let input = document.getElementById('eth_min_premium');
            let currentValue = parseFloat(input.value);
            if (currentValue >= 0.01) input.value = (currentValue - 0.01).toFixed(2);
        }
        function incrementEthFilter() {
            let input = document.getElementById('eth_min_premium');
            let currentValue = parseFloat(input.value);
            if (currentValue <= 499.99) input.value = (currentValue + 0.01).toFixed(2);
        }
...
def update_system4_cooldown():
    ...
    try:
        ...
        if new_cooldown < 5: new_cooldown = 5
...
def update_system4_btc_filter():
    ...
    try:
        ...
        if new_filter < 0: new_filter = 0
        if new_filter > 1000: new_filter = 1000
...
def update_system4_eth_filter():
    ...
    try:
        ...
        if new_filter < 0.01: new_filter = 0.01
        if new_filter > 500: new_filter = 500
...
def start_system5_btc_call():
    ...
    try:
        ...
        if not strike_str: return redirect('/?success=Please+select+a+strike')
...
def health():
    ...
    return {
        ...
        "system_2_option_alerts": {
            ...
            "configs": {config_id: asdict(config) for config_id, config in alert_configs.items()},
            "last_check": last_check_time.isoformat() if last_check_time else None
        },
        "system_3_spike_detector": {
            "condition_1_spike": {"active": spike_config.enabled_spike, "min_spike_percent": spike_config.min_spike_percent, "min_premium": spike_config.spike_min_premium},
            "condition_2_spread": {"active": spike_config.enabled_spread, "min_spread_percent": spike_config.min_spread_percent, "min_premium": spike_config.spread_min_premium}
        },
        ...
        "system_5_premium_tracker": {config_id: asdict(config) for config_id, config in premium_tracker_configs.items()},
...
def start_bots():
    ...
    print("="*60)
    print(f"⚡ System 1: Arbitrage Alerts (BTC: ${DELTA_THRESHOLD['BTC']}, ETH: ${DELTA_THRESHOLD['ETH']})")
    print(f"🎯 System 2: Option Strike Alerts")
    print(f"🚨 System 3: Dual Condition Spike Detection")
    print(f"🎯 System 4: Exact Premium Match Detection (BTC Filter: ${premium_match_config.btc_min_premium}, ETH Filter: ${premium_match_config.eth_min_premium})")
